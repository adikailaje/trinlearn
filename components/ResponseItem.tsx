...
interface ResponseItemProps {
  ...
  onSaveMachine?: (originalGeminiOutput: string) => Promise<void>;
  onClick?: (response: GeminiResponse) => void; // New prop for click handling
}
...
export const ResponseItem: React.FC<ResponseItemProps> = ({ response, onSaveMachine, onClick }) => {
  ...
  const canSave = response.make && response.modelName && !response.isError && onSaveMachine;
  ...
  const handleSaveClick = async (e: React.MouseEvent) => {
    e.stopPropagation(); // Prevent click from bubbling to the parent if it's also clickable
    if (!canSave || !response.originalGeminiOutput) return;
    setIsSaving(true);
    setJustSaved(false);
    try {
      await onSaveMachine(response.originalGeminiOutput);
      setJustSaved(true);
        ...
        {canSave && (
          <div className="mt-3">
            ...
            >
              ...
              {isSaving ? 'Saving...' : (justSaved ? 'Saved!' : 'Save Machine')}
            </button>
          </div>
        )}
      </div>
